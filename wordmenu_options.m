...
% end
% %istiyorum ki sağ oka bastığında bi sonraki kelimeye geçsin (R+1) sol oka
% %bastığımda bi öncekine geçsin (r-1).
% % if presskeyrightarrow
% % count = count + 1
% 
...
% 
% % Create an array of 26 page windows
% pageWindows = cell(26, 1);
% for i = 1:26
%     pageWindows{i} = Screen('OpenOffscreenWindow', window, [0 0 0], windowRect);
%     Screen('TextSize', pageWindows{i}, 24);
%     DrawFormattedText(pageWindows{i}, sprintf('Page %d of 26', i), 'center', 'center', [255 255 255]);
% end
% 
% currentPage = 1;
% 
...
%     Screen('CopyWindow', pageWindows{currentPage}, window);
%     Screen('Flip', window);
%     
...
%         % Go to the next page
%         currentPage = min(26, currentPage + 1);
...
% pages = cell(26, 1);
% for i = 1:26
%     pages{i} = sprintf('Page %d of 26', i);
% end
% 
% currentPage = 1;
% 
% while true
%     % Display the current page
%     DrawFormattedText(window, pages{currentPage}, 'center', 'center', [255 255 255]);
%     Screen('Flip', window);
%     
%     % Wait for a key press
%     [~, ~, keyCode] = KbCheck;
